...
float getTrilinearInterpolation(float* values, float endPoints[8][3], float* point){
    ...
    pt1[0] = point[0] - endPoints[0][0]; pt1[1] = point[1] - endPoints[0][2];    
...
float getValueTriLinear(ScalarField *field, float *point){
    ...
    reqIndex[1] = 0;
    reqIndex[2] = 0;
    for (int i = 0; i<3; i++) {endPoints[0][i] = (float)reqIndex[i]*(field->step[i]);}
    ...
    return getTrilinearInterpolation(values , endPoints, point);
...
int main(int argc, char ** argv) {
    // testing code
    int i, j;
    ScalarField *field = loadField("1duk.pot");
    ...
    float points[8][3];
    float values[8];
    float point[3] = {-3.623750e+01 -2.128000e+00 -4.185750e+01};
    int in = 0;
    for(int z = 0; z < 2; z++){
        for(int y = 0; y < 2; y++){
            for(int x = 0; x < 2; x++){
    ...
    float interp = getValueTriLinear(field, point);
    printf("The value at point mentioned  = %f", interp);
